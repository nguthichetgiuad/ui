...
Library.Create = function(_, LibraryOptions)
	...
	local Gui = self:Object("ScreenGui", {
		...
		Name = "MainFrame",
		Parent = Gui, -- ĐẢM BẢO LÀ Gui, KHÔNG PHẢI ScreenGui
		AnchorPoint = Vector2.new(0.5, 0.5),
		BackgroundTransparency = 0, -- Chỉnh về 0 để nó hiện ra ngay
		BackgroundColor3 = Color3.fromRGB(30, 30, 30),
		Position = UDim2.new(0.5, 0, 0.5, 0),
		Size = LibraryOptions.Size -- Cho nó hiện size thật luôn, khỏi đợi Animation lỗi
	})
	...
	TabsLibrary.Quit = function(_, QuitOptions)
		...
		for i, v in next, Tabs:GetChildren() do
			if v:IsA("TextButton") then
				task.spawn(function()
					TS:Create(v:FindFirstChild("NameLabel"), TI(0.15), {TextTransparency = 1}):Play()
					task.wait(0.15)
					v:Destroy()
				end)
			end
